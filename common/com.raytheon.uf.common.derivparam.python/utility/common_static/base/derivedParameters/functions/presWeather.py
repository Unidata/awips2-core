...
# ------------- -------- --------- --------------------------------------------
# Jun 26, 2019  7880     tgurney   2to3 fixes for derived parameters
# Mar 14, 2022  8815     randerso  Fix string dtype for Python 3
##
## @file presWeather.py
...
def execute1(QPF6hr_bestCat, obVis_bestCat, precipType, tstorm6hr, severe6hr):
    ...
    obsTypeAtAll = zeros(obVis_bestCat.shape, dtype='|U10')
    wxTypeAtAll = zeros(precipType.shape, dtype='|U10')
    ...
    storms = zeros(severeYes.shape, dtype='|U10')
    storms[tstormYes] = 'TS'
    storms[severeYes] = '+TS'
...
def typeTransform(dataArray, lookup):
    sDataArray = dataArray.astype('|U10')
    for value in lookup:
