 ...
 * Aug 02, 2013  2190     mschenke  Changed "panes" to Map to simplify code
 * Feb 09, 2015  4092     bsteffen  Add bundle listener to trigger shutdown.
 * Jun 05, 2019  64619    tjensen   Remove Float division for fpsThrottle calc
 *                                  to avoid float precision rounding
 *
 * </pre>
 *
 * @author chammack
...
public class DrawCoordinatorJob extends Job {
    ...
    private final int fpsThrottle = VizApp
            .getCorePreferenceInt(PreferenceConstants.P_FPS);

    /** The amount of time to sleep between frames */
    private final long sleepTime = TimeUtil.MILLIS_PER_SECOND / fpsThrottle;
     ...
     */
    private final int frameSkipLimit = (int) (segmentCheckTime / sleepTime
            * 0.2);
    ...
    private final Map<IDisplayPaneContainer, DrawCoordinatedPane> panes = new IdentityHashMap<>();
    ...
    protected IStatus run(IProgressMonitor monitor) {
        ...
        final ArrayList<DrawCoordinatedPane> listToDraw = new ArrayList<>();
        ...
        while (!monitor.isCanceled()) {
            try {
                ...
                synchronized (panes) {
                    managed = new HashSet<>(panes.values());
                }
                ...
                if (listToDraw.size() > 0) {
                    ...
                    if (sleepDelta > 0) {
                        ...
                        framesSkippedInLastSegment += (int) (-sleepDelta
                                / sleepTime);
