 ...
 * May 15, 2018 6978       tgurney      Run only if the corresponding radio
 *                                      button is selected
 *
 * </pre>
 *
        ...
        if (descriptor == null) {
            for (LocalizationFileEntryData file : files) {
                LocalizationEditorUtils.openInEditor(page,
                        new LocalizationEditorInput(file.getFile(),
                                file.getResource()));
            }
        } else if (isChecked()) {
            for (LocalizationFileEntryData file : files) {
                LocalizationEditorUtils
                        .openInEditor(page,
                                new LocalizationEditorInput(file.getFile(),
                                        file.getResource()),
                                descriptor.getId());
                LocalizationEditorUtils.getEditorRegistry()
