...
public class ImportFileAction extends Action {
    ...
    public static boolean importFile(List<LocalizationType> contextTypes,
        ...
        List<File> applicable = new ArrayList<>(importFiles.length);
        ...
        List<ILocalizationFile> localizationFiles = new ArrayList<>(
                applicable.size());
        List<ILocalizationFile> existing = new ArrayList<>(
                applicable.size());
        ...
        for (File importFile : applicable) {
            ...
            if ((lf != null) && !ProtectedFileLookup.isProtected(lf)) {
        ...
        List<ILocalizationFile> skip = new ArrayList<>();
        if (!existing.isEmpty()) {
            if (existing.size() > 1) {
    ...
    private static class MultiConfirmDialog extends MessageDialog {
        ...
        public MultiConfirmDialog(List<ILocalizationFile> existingFiles) {
            ...
            this.confirmedFiles = new ArrayList<>(
                    existingFiles.size());
